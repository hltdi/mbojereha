...
class Language:
    ...
    aliases = {'e': 'spa', 'es': 'spa', 'g': 'grn', 'gn': 'grn'}
...
#        print("Lengua {} creada".format(self))
    ...
    def get_abbrev(code):
        """
        Code could be the actual abbrev or an alias.
        """
        return Language.aliases.get(code, code)
    ...
    def postag_match(self, taggertag, morphotag):
        """
        Does the POS tag from the tagger match the tag from the morphological
        analyzer?
        """
...
#            print("{}: cached pref {} agrees with tag".format(word, tag))
    ...
    def read_groups(self, posnames=None, target=None, reverse=True,
                    verbosity=0):
        """
...
#                     lemmas=d.get('lemmas'),
    ...
    @staticmethod
    def load_trans(source, target, bidir=True):
        """
...
#                    targlang.read_groups(target=srclang)
    ...
    def load_morphosyntax(self, target, reverse=True):
        """
    ...
    def translate_special(self, form):
        ...
        Default is to just return the form.
        """
